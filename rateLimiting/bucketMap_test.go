...
// Tests that LookupBucket() returns the correct bucket or creates it if it does
// not exist. This is done by adding tokens to buckets that are added and
// checking if the number of tokens match when looking up the bucket.
//
...
// Tests that LookupBucket() can lookup an existing bucket while a read lock is
// enabled.
...
// Tests that LookupBucket() cannot lookup an existing bucket while there is a
// write lock.
...
// Tests that LookupBucket() cannot add add a new bucket to the map while a read
// lock is enabled.
...
// Tests that AddBucket() correctly adds new buckets and overwrites existing
// buckets. This function runs twice, once without a database and a second time
// with a database.
...
// Tests that AddBucket() cannot add add a new bucket to the map while a read
// lock is enabled.
...
// Tests that addAllBuckets() adds all the buckets with the correct values.
...
// Tests that AddToWhitelist() correctly adds the whitelisted items or modifies
// existing buckets to be whitelisted, if they are already in the map. This
...
// Tests that AddToWhitelist() cannot mark buckets as whitelisted while a read
// lock is enabled.
...
// Tests that DeleteBucket() deletes the correct buckets. This function runs
// twice, once without a database and a second time with a database.
...
// Tests that DeleteBucket() returns an error when the bucket does not exist.
...
// Tests that DeleteBucket() cannot delete buckets from the map while a read
// lock is enabled.
...
// Tests that StaleBucketWorker() deletes stale buckets in a separate thread and
// that the quit channel stops it.
...
// Tests that clearStaleBuckets() removes all the stale buckets from the map.
...
// Tests that clearStaleBuckets() can search for stale buckets during a read
// lock.
...
// Tests that clearStaleBuckets() cannot search for stale buckets during a write
// lock.
...
// Tests that clearStaleBuckets() cannot remove stale buckets from the map while
...
// Tests that createAddToDbFunc() generates an anonymous function that panics
// when it attempts to operate on a bucket that does not exist in the database.
